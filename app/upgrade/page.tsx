  ...
  return (
    ...
    >
      ...
      <div className="flex items-center mb-4">
        <div className="mr-4 text-yellow-400 bg-yellow-400/10 p-3 rounded-full border border-yellow-400/20">{icon}</div>
        <h3 className="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-yellow-300 to-yellow-500">{title}</h3>
      </div>
      <p className="text-gray-300 mb-4 text-sm">{description}</p>
      <div className="flex justify-between items-center mb-4 bg-black/20 rounded-xl p-3 border border-yellow-500/5">
        <span className="text-gray-400">{t('currentLevel')}: <span className="text-white font-bold text-lg">{currentLevel}</span></span>
        <span className="text-gray-400">{t('upgradeCost')}: 
      ...
      <div className="flex justify-between items-center mb-4 bg-black/20 rounded-xl p-3 border border-yellow-500/5">
        <span className="text-gray-400">{t('currentEffect')}: <span className="text-white font-bold text-lg">{currentEffect}</span></span>
        <span className="text-gray-400">{t('nextEffect')}: <span className="text-emerald-400 font-bold text-lg">{nextEffect}</span></span>
      </div>
      <motion.button
...
const UpgradePageContent: React.FC = React.memo(() => {
  ...
  const capacityCost = useMemo(() => Math.floor(100 * Math.pow(1.1, (gameState.inventory?.containerCapacityLevel || 1) - 1)), [gameState.inventory?.containerCapacityLevel]);
  const speedCost = useMemo(() => Math.floor(100 * Math.pow(1.5, gameState.fillingSpeedLevel - 1)), [gameState.fillingSpeedLevel]);
