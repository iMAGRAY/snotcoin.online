...
/**
 * Инициализирует состояние игры для пользователя
 * @param userId ID пользователя
 * @returns Результат инициализации
 */
export async function initializeGameState(
  userId: string
): Promise<{ success: boolean; data?: ExtendedGameState; error?: string }> {
  try {
    console.log(`[DataService] Инициализация состояния игры для пользователя ${userId}`);
    
    // Сначала попробуем загрузить существующее состояние
    const existingState = await loadGameState(userId);
    
    if (existingState) {
      console.log(`[DataService] Найдено существующее состояние для ${userId}`);
      return {
        success: true,
        data: existingState
      };
    ...
    console.log(`[DataService] Создание нового состояния для ${userId}`);
    
    // Импортируем функцию создания начального состояния игры
    const { createInitialGameState } = await import('../constants/gameConstants');
    
    // Создаем начальное состояние
    const initialState = createInitialGameState(userId);
    
    // Устанавливаем дополнительные поля
    initialState._userId = userId;
    initialState._createdAt = new Date().toISOString();
    initialState._lastModified = Date.now();
    initialState._saveVersion = 2; // Используем последнюю версию формата
    
    // Сохраняем созданное состояние
    await saveGameState(userId, initialState, true);
    ...
    console.error(`[DataService] Ошибка при инициализации состояния:`, error);
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Неизвестная ошибка'
    };
