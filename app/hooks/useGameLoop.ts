...
export const useGameLoop = (isPaused: boolean) => {
  ...
  const gameLoop = useCallback(() => {
    ...
    let updatedBalls = thrownBalls.map(updateBallPhysics);
    ...
    for (let i = 0; i < updatedBalls.length; i++) {
      for (let j = i + 1; j < updatedBalls.length; j++) {
        if (detectCollision(updatedBalls[i], updatedBalls[j])) {
          ...
          if (mergedBall) {
            updatedBalls = updatedBalls.filter(ball => ball.id !== resolvedBall1.id && ball.id !== resolvedBall2.id);
  ...
  }, [isPaused, thrownBalls, updateBallPhysics, updateBalls, increaseScore, updateHighestMergedLevel]);
