...
const CoinImage = React.memo(() => (
  <Image
    src="https://hebbkx1anhila5yf.public.blob.vercel-storage.com/Coin-isXsRIReOu97eVUBK3mchT3LRt7fj6.png"
    alt="Coin"
    width={40}
    height={40}
...
const FallingReward = React.memo(({ reward, uniqueId, rewardColor }: { reward: Reward; uniqueId: number; rewardColor: string }) => {
...
const FallingRewards: React.FC<FallingRewardsProps> = React.memo(({ amount, onComplete, uniqueId, rewardColor }) => {
  ...
  const renderReward = useCallback((reward: Reward) => (
    <FallingReward key={reward.id} reward={reward} uniqueId={uniqueId} rewardColor={rewardColor} />
