...
const PauseMenu: React.FC<PauseMenuProps> = ({ togglePause, handleRestart, handleHome, gamesAvailable }) => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <p className="text-white text-xl mb-4">{t('gamesAvailable')}: {gamesAvailable}</p>
        {[
          { text: t('resume'), icon: Play, onClick: togglePause, color: 'from-green-500 to-green-600' },
          { text: t('restart'), icon: RefreshCw, onClick: handleRestart, color: 'from-blue-500 to-blue-600' },
          { text: t('exit'), icon: Home, onClick: handleHome, color: 'from-red-500 to-red-600' },
        ].map((button, index) => (
          <motion.button
            key={index}
