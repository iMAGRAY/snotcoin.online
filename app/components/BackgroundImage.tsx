...
const collectButtons = [
  ...
  { color: "from-yellow-400 to-yellow-500", text: "150%", riskText: "Risky", multiplier: 1.5, successChance: 0.70 },
  { color: "from-red-400 to-red-500", text: "200%", riskText: "Very risky", multiplier: 2, successChance: 0.40 },
...
const BackgroundImage: React.FC = React.memo(() => {
  ...
  }, [consumeEnergy, dispatch, containerSnot, maxContainerSnot, fillingSpeed, lastClickTime, flyingNumberId, localState.flyingNumbers.length, setContainerScale]);
  ...
  const handleColorSelect = useCallback((button: typeof collectButtons[0]) => {
    ...
    const amountToCollect = isSuccess ?
      Math.floor((button.multiplier === 1 ? Math.floor(containerSnot) : Math.round(Math.floor(containerSnot) * button.multiplier)) * collectionEfficiency) : 0;
    ...
    if (isSuccess) {
      ...
      localDispatch({ type: 'SET_LOCAL_STATE', payload: {
  ...
  }, [containerSnot, collectionEfficiency, dispatch]);
    ...
    >
      {collectButtons.map((button, index) => (
        <motion.button
          ...
          whileTap={{ scale: 0.98 }}
        >
          <span className="relative z-10 flex flex-col items-center justify-center" style={{ textShadow: '2px 2px 0 rgba(0,0,0,0.3)' }}>
  ...
  const renderCollectButton = useMemo(() => (
    ...
    >
      <span className="relative z-10 flex items-center justify-center tracking-wider" style={{ textShadow: '2px 2px 4px rgba(0,0,0,0.5)' }}>
        {t('collect')}
        <Sparkles className="ml-2 h-5 w-5" />
      </span>
      <div className="absolute inset-0 bg-gradient-to-b from-white/20 to-transparent" />
      {containerSnot >= maxContainerSnot && (
        <div className="absolute inset-0 rounded-xl" style={{ boxShadow: 'inset 0 0 20px 5px rgba(255,215,0,0.5)' }} />
      )}
    </motion.button>
  ...
  const MemoizedResources = useMemo(() => (
    <Resources
      snotCoins={isNaN(inventory.snotCoins) ? 0 : inventory.snotCoins}
      snot={isNaN(inventory.snot) ? 0 : inventory.snot}
    ...
    />
  ), [inventory.snotCoins, inventory.snot, energy, maxEnergy]);
    ...
    <div className="h-full w-full relative overflow-hidden">
      ...
      <div className="absolute bottom-8 left-0 right-0 z-50 px-4">
        <div className="w-full max-w-md mx-auto flex space-x-2">
          <AnimatePresence>
            {localState.showColorButtons ? renderColorButtons : (
              <>
